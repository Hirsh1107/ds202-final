...
# Select the columns to be used for finding similar beers
selected_columns <- c('Sweet', 'Sour', 'Salty', 'Bitter') # Add more columns if needed

# Create beer_data_norm
beer_data_norm <- as.data.frame(lapply(data[, selected_columns], normalize))

# Find the row number of "Long Trail Ale"
user_beer_row <- which(data$`Name` == "Long Trail Ale")

# Check if "Long Trail Ale" exists in the data
if(length(user_beer_row) == 0) {
  print("The beer 'Long Trail Ale' does not exist in the data.")
} else {
  # Get the characteristics of "Long Trail Ale"
  user_beer <- beer_data_norm[user_beer_row, ]

  # Remove "Long Trail Ale" from the data
  ...
  #print(distances)
  closest_beers <- order(distances)[1:5]
  print(closest_beers)
